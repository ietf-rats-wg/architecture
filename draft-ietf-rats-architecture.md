...
A second approach places the onus of timekeeping solely on the remote entity (e.g., the Verifier for Evidence, or the Relying Party for an Attestation Result).
...
In this approach, a non-predictable nonce is generated by the remote entity, and the nonce is then signed and included along with the Claims in the Evidence or Attestation Result.
After checking the sent and received nonces are the same, the remote entity knows that the Claims were signed after the nonce was generated, effectively locking its clock to the produced payload.
...
Whatever the approach, it is important to note that the actual values in Claims might have been generated long before the Claims are signed.
If so, it is the signer's responsibility to ensure that the values are still correct when they are signed.
For example, values might have been generated at boot, and then used in Claims as long as the signer can guarantee that they cannot have changed since boot.
