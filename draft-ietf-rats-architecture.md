...
in Composite Devices {{compositedevice}}. Most importantly, these conveyance methods are
...
A third option to establish the epoch of Evidence or Attestation Result
generation - next to nonces and timestamps ({{freshness}}) - is by using
"handles".

Handles are opaque, non-predictable data regularly broadcast by a Handle Distributor.  
Each handle represents a specific epoch,
in which generated Evidence can be asserted to be fresh.

The distribution of a new handle determines the transition from one epoch to
the next.  The transition between epochs goes through an "overlap" phase in
which the previous epoch is still extant while the current epoch is
established.  The span of the overlap phase is determined solely by the
transmission delay of the underlying communication channel,
`time(HR)-time(HT)`.  Once the overlap phase is over, an epoch is fully
established.

In order to determine freshness of Evidence or Attestation Results, consumers
of messages based on handles need to understand whether an epoch is fully
established or in overlap phase.

The Handle Distributor, hosted in a dedicated entity or co-located with other
RATS roles, is trusted by the protocol participants to behave correctly with
regards to its duty of handle distribution.

Consumers of conceptual messages based on handles must be able to safely trace
each handle's origin back to the broadcasting Handle Distributor either using
topological considerations (e.g., co-location), or by cryptographic means.

The example in {{fig-handle-passport}} illustrates a Handle-based interaction
using the passport topology model.

At `time(HT)`, the Handle Distributor broadcasts handle `H` which starts a new
...
definition provided above, and generates an Attestation Result.  The
Attestation Result is conveyed to the Attester.

After the transmission of handle `H'` at `time(HT')` a new epoch `E'` is
...
~~~~
   ...
   '----------'   '-------------'   '----------'  '---------------'
     time(VG_a)          |                |               |
        ...
        ~                ~                ~               ~
        |                |                |               |
     time(HR_a)<----time(HT)-----------time(HR_v)----->time(HR_r)
        |                |                |               |
     time(EG_a)          |                |               |
        |--E{H,time(VG_a)}--------------->|               |
        |                |                |               |
        ...
        |<----------------R{H,time(RX_v)}-|               |
        |                |                |               |
     time(HR'_a)<---time(HT')----------time(HR'_v)---->time(HR'_r)
        |                |                |               |
     time(RR_a)          |                |               |
