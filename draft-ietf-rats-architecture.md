...
maintain confidentiality of the public key: however integrity of the chain of custody of the public key is necessary in order to avoid attacks where an attacker is able to enroll an attestation key that they control.

To summarize: attestation key provisioning must ensure that only attestation key material that is generated by a valid Endorser is established in Attesters.
