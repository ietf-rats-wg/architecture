...
* Appraisal Policy: A set of rules that direct how a system evaluates
...
* Attestation: A process by which one entity (the "Attester") provides evidence
    about its identity and health to another entity, which then assesses its
    trustworthiness.

* Attestation Result: The evaluation results generated by a Verifier,
    typically including information about an Attester, where the
...
* Endorser: An entity that creates Endorsements that can be used to help evaluate
    trustworthiness of Attesters.

* Evidence: A set of information about an Attester that is to be evaluated
    by a Verifier.
