...
In this case, the device desiring access to a network must produce attestation for consideration by a network infrastructure device such as a router, switch, or access point
...
In this case, device desiring to run an ML model must produce attestation for consideration by a server or service holding ML models it desires to protect
...
In this case, an entity desiring to retrieve confidential data must produce attestation for consideration by an entity that holds confidential data for retrieval by other entities
...
In this case, a device or application wishing to control physical equipment must produce attestation for consideration by a device or application connected to potentially dangerous physical equipment (hazardous chemical processing, traffic control, power grid, etc.)
...
In this case, a device with a trusted execution environment capable of running trusted applications that can be updated  must produce attestation for consideration by a Trusted Application Manager
...
In this case,  the device that is desired to keep from being held hostage for a long period of time must produce attestation for consideration by a remote server that will securely grant the Attester permission to continue operating (i.e., not reboot) for a period of time
...
In this case, every FIDO Authenticator must produce attestation for consideration by any web site, mobile application back end or service that does biometric authentication.
  
### Use case terminology
In the above use cases, the entity producing attestation is called the "Attester" and the entity considering the attestation is called the "Relying Party". The following sections explore this more formal terminology.
...
Negotiates: the relationship between an Attester and a Verifier
