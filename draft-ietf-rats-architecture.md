...
such as by collecting, protecting or signing measurements.
Measurements that have been signed by a trusted component comprises Evidence that either supports or contradicts a claim of trustworthiness.
...
The bottom layer of an Attester has an Attesting Environment that is implicitly trusted, typically because it is designed to be immutable or difficult to modify by malicious code.
...
In layered attestation, an implicitly trusted Attesting Environment is the initial layer.
Claims can be collected from or about each layer beginning with an initial layer.
...
Claims about an implicitly trusted layer typically are asserted by an Endorser.
