...
# Appendix B: System Examples

(Rough first draft to see if there is interest)

## System Example with OS Vendor as Main Driver

Three entities (companies / organizations) participate in this examples.

The operating system vendor is at the center. It sets the architecture and the level of security achieved. It performs the following:
...
* Physically makes the device to OS vendors security requirements
* Installs the OS on the device
* Personalizes each device with a unique private key and certificate supplied by the OS vendor

The application vendor that implements some application with a back-end cloud service. It performs the following:
* Author an app on the device that calls the attestation API to obtain Attestation Evidence
* Convey the Attestation Evidence to the back-end cloud service over the network
* Verify the Attestation Evidence and decided whether or not to allow application service

This maps as follows in the RATS architecture:

Endorser:

: The operating system vendor is the endorser

Endorsement:

: The root certificate published by the endorser

Attester:

: The HW made by the device OEM and the SW made by the OS vendor

Attestation Evidence:

: A protocol message generated by the Attester on the request of the application that is then sent to the application vendor cloud service.

Verifier and Verifier Owner:

: are run by the application vendor cloud service

Relying Party and Relying Party Owner:

: are also the application vendor cloud service; since the Verify and Relying Part are the same the distinction between passport and background check doesn’t apply.

Attestation Result:

: non existent as the Verifier is the same as the Rely Party


## System Example Made Up of Chip Vendor and IoT Device Maker

Two entities (companies / organizations) participate in this example.

The chip vendor that makes the chip. They also make the SW that is the Attester. It does the following:
* Manufacturers the chip
* Authors the SW that runs on the chip that is the Attester
* Personalizes each chip with a unique 256-bit symmetric key and key ID
* Also stores the 256-bit symmetric key in a well-secured database
* Operates a web-based HMAC verification service the inputs to which are the key ID and HMAC to verify and the output is a yes or no on the verification

The IoT device maker buys chips from the vendor, makes the chip the core of an IoT device (perhaps sensors for a home security service). It does the following:
* Manufacturers and a device that an end user may purchase
* Adds their own SW to the device over the top of the SW the chip vendor supplied
* Operates a cloud-based service with which the device interacts
* An Attestation Evidence is included whenever the IoT device communicates with the cloud service
* The cloud service decodes the Attestation Evidence to get the key ID and computes the HMAC for it
* The close service sends the key ID and HMAC to the chip vendors verification service

This maps as follows in the RATS architecture:

Endorser:

: The chip vendor is the Endorser

Endorsement:

: The Endorsement only exists virtually inside the chip vendor

Attester:

: The HW and SW made by the chip vendor

Attestation Evidence:

: A protocol message generated by the Attester sent to the cloud service operated by the IoT device vendor

Verifier and Verifier Owner:

: The Verifier and Verifier Owner are split with part run by the chip vendor and part run by the IoT device maker
...
## System Example Base on a Third Party Verification Service

Three entities (companies / organizations) participate in this example:

First off, there is a device maker. It does the following:
* Manufacturers devices using HW and SW of their choice that includes an Attester
* Puts an attestation key into each device (symmetric or asymmetric)
* Has the device certified by one or more certification programs
* Generates a detailed Endorsement for each device that includes the verification key, list of certifications, reference measurement values and implicit claims

The Relying Party operates some cloud service for which attestation needed, perhaps a payment service. It does the following:
* Receives service requests from devices. The service requests includes Attestation Evidence.
* Sends the Attestation Evidence to a 3rd party verification service.
* Receives Attestation Results from 3rd party verification service.
* Decides whether to grant the service, perhaps varying the payment amount allowed based on details that are in the Attestation Result such the certifications the device has received

The 3rd Party Verification and Attestation Aggregation Service. It does the following:
* Receives Attestation Evidence from the Relying Party (This uses the Background Check Model)
* Looks up the Endorsement by an Endorsement identifier, perhaps a URI, and receives the Endorsement from the device maker, most likely over a channel that has both confidentiality, integrity protection and authentication.
* Performs the Verification of the Attestation Evidence which involves: verifying the signature on the Attestation Evidence and comparison of claims to reference values in the Endorsement.
* Generates the Attestation Results which involves making some new values that report the result of comparison to the reference values and copying implied claims from the Endorsements and copying some claims from the Attestation Evidence
...
: A detailed message created by the device maker that is secured for confidentiality that contains reference values, the verification key and implicit claims

Attester: 

: The HW and SW made by the device maker

Attestation Evidence:

: A protocol message generated by the Attester sent to the Relying Party

Verifier and Verifier Owner:

: A third-party Attestation service that can process Attestation Evidence from many different device makers and provide results to many different Relying Parties.

Relying Party and Relying Party Owner:

: The operator of a cloud-based payment service.

Attestation Result:

: A detailed set of claims with 1) claims copied forward from the Attestation Evidence , 2) implied claims copied through from the Endorsement and 3) new claims created to report the result of comparison to reference values
